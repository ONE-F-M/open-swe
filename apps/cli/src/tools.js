...
export const benchMigrateTool = tool(async ({ site }) => {
    ...
    let stderr = '';
    try {
        const result = await execaCommand(`bench --site ${site} migrate`, { cwd: "/Users/samdani/Desktop/onefm_bench" });
...
export const benchRunTestTool = tool(async ({ app, module }) => {
    ...
    let stderr = '';
    try {
        const result = await execaCommand(cmd, { cwd: "/Users/samdani/Desktop/onefm_bench" });
        output = result.stdout;
        stderr = result.stderr;
        exitCode = result.exitCode ?? 0;
...
export const benchClearCacheTool = tool(async ({ site }) => {
    ...
    let stderr = '';
    try {
        const result = await execaCommand(`bench --site ${site} clear-cache`, { cwd: "/Users/samdani/Desktop/onefm_bench" });
        output = result.stdout;
        stderr = result.stderr;
        exitCode = result.exitCode ?? 0;
    }
...
export const benchConsoleTool = tool(async ({ site, code }) => {
...
export const benchGetDocInfoTool = tool(async ({ site, doctype, name, field, doctypeDef }) => {
    ...
    if (doctypeDef) {
        ...
        try {
            const result = await execaCommand(cmd, { cwd: "/Users/samdani/Desktop/onefm_bench", shell: true });
            stdout = result.stdout;
            stderr = result.stderr;
            exitCode = result.exitCode ?? 0;
            try {
